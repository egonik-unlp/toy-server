...
async fn some_kinda_handler(req: &Request) -> OuterHashmap {
...
async fn main() -> Result<(), ServerError> {
    let address = match std::env::args().nth(1) {
        Some(port) => {
            let _ = port
                .parse::<i32>()
                .map_err(|_| std::io::Error::new(std::io::ErrorKind::InvalidData, "Ivalid port"));
            println!("HAI");
            format!("0.0.0.0:{}", port)
        }
        None => {
            ...
            println!(
                "As no port is provided via env args, using default {}",
                falopa
            );
            falopa.into()
        }
