...
fn parse_headers(stream: &mut BufReader<TcpStream>) -> Result<Headers, RequestError> {
    ...
    loop {
        let line = read_first_line(stream)
            .map_err(|err| RequestError::new(format!("Error parseando headers: {}", err)))?;
        if line.len().eq(&0) {
            break;
        }
        ...
        let header_name = parts
            .next()
            .ok_or_else(|| RequestError::new("Error parseando nombre de header".into()))?;
        let content = parts
            .next()
            .ok_or_else(|| RequestError::new("Error parseando valor de header".into()))?;
